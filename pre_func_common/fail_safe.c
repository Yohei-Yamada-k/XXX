...
void f_CANFailInfo_set(MiconType Micon)
{
    /* [ID Table] TC Struct Elements(Array) */
    ...
    /* [ID Table] VMC Struct Elements(Array) */
    uint8_t Can_Recv_ID_VMC_Array[VMC_ID_NUM] = { CanID4 };

    /* [ID Table] Aurix Struct */
    ...
    uint8_t* API_Result[2] = { API_Result_TC, API_Result_VMC };

    f_CANRecv_set(Micon, Can_Recv_ID_Array, API_Result);
}
...
/*=================================================================================================================*/
    ...
    for(; i < CanID[Micon].size; i++){
        ...
        /* [ Get API ] */
        API_Array[Micon][i] = Stub_CallAPI_CanRecv(CanID[Micon].array[i]);

        /* [ PROHIBITION ] */
        if(!(Can_Recv_Fail_Falg[Micon][i] == Flag_Off))
        {

        }
        else if(power_On < Power_On_thr){
            if(!(Can_Recv_Err_Cnt[Micon][i] == Err_Off))
            {
                Can_Recv_Err_Cnt[Micon][i] = 0;
            }
            else
            {
                // Do Nothing
        ...
        {
            /* [ FAIL JUDGMENT ] */
            if(!(API_Array[Micon][i] == Err_Off)){

                // Err Count Up!
                Can_Recv_Err_Cnt[Micon][i]++;

                if(Can_Recv_Err_Cnt[Micon][i] >= v_CanRecvTh){
                    
                    /* [TBD : SR_REQUIREMENTS] : Set Fail Flag */ 
                    Can_Recv_Fail_Falg[Micon][i] = Flag_On;

                    TmporaryFlag = Can_Recv_Fail_Falg[Micon][i];

                    /* [TBD : SR_REQUIREMENTS] : Set Fail State Condition */
                    if(Can_Recv_Fail_Falg[Micon][i]){
            ...
            {
                if(!(Can_Recv_Err_Cnt[Micon][i] == Err_Off)){
                    Can_Recv_Err_Cnt[Micon][i] = Err_Off;
    ...
    if(!(TmporaryFlag == Flag_Off) && Can_Recv_Fail_Falg[Micon][i] == Flag_Off){
        Can_Recv_Fail_Falg[Micon][i] = Flag_On;
    }
